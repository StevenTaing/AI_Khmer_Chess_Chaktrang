...
export const minimax = (node, depth, alpha, beta, isMaximizingPlayer) => {
    ...
    for (const child of children) {
        const score = minimax(child.childNode, depth - 1, alpha, beta, !isMaximizingPlayer).score;
...
const evaluateNode = (node) => {
    ...
    for (const childNode of node) {
        const value = pieceScores[childNode.piece] | 0;
...
const getChildren = (node, isMaximizingPlayer) => {
    ...
    for (const poistion of node) {
        ...
        if (getPlayer === originPlayer) {
            let getPossibleMove = determine(node, getPiece, getPlayer, getRow, getColumn);
