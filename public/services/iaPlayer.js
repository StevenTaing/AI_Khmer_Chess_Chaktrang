...
export const aiMove = () => {
    ...
    const bestMoveResult = minimax(boardStates(), depth, -Infinity, Infinity, false);
    const bestMoves = bestMoveResult.move;
    if (bestMoves) {
        const randomIndex = Math.floor(Math.random() * bestMoves.length);
        const move = bestMoves[randomIndex];
        makeMove(move.originCell, move.originPlayer, move.originPiece, move.destinationcell);
    }
};
